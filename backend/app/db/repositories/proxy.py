...
class ProxyRepository(BaseRepository[Proxy]):
    ...
    async def get(self, db: AsyncSession, proxy: PyrogramProxy) -> Optional[Proxy]:
        result = await db.execute(select(self.model).where(self.model.protocol == proxy.scheme, self.model.ip == proxy.hostname, self.model.port == proxy.port, self.model.username == proxy.username, self.model.password == proxy.password))
        return result.scalars().first()
